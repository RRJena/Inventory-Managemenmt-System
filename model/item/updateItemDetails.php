<?php
	...
	if(isset($_POST['itemNumber'])) {
		...
		$min_stock = htmlentities($_POST['min_stock']);
		$max_stock = htmlentities($_POST['max_stock']);
		...
		if(!empty($itemNumber) && !empty($itemName) && isset($itemDetailsQuantity) && isset($itemDetailsUnitPrice)){
			...
			$updateItemDetailsSql = 'UPDATE item SET itemName = :itemName, discount = :discount, stock = :stock, unitPrice = :unitPrice, status = :status, min_stock= :min_stock, max_stock= :max_stock, description = :description WHERE itemNumber = :itemNumber';
			$updateItemDetailsStatement = $conn->prepare($updateItemDetailsSql);
			$updateItemDetailsStatement->execute(['itemName' => $itemName, 'discount' => $discount, 'stock' => $newStock, 'unitPrice' => $itemDetailsUnitPrice, 'status' => $status,'min_stock' => $min_stock,'max_stock' => $max_stock, 'description' => $description, 'itemNumber' => $itemNumber]);
