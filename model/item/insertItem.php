	...
	if(isset($_POST['itemDetailsItemNumber'])){
		...
		if(!empty($itemNumber) && !empty($itemName) && isset($quantity) && isset($unitPrice)){
			...
			if($stockStatement->rowCount() > 0){
				...
				$insertItemSql = 'INSERT INTO item(itemNumber, itemName, discount, stock, unitPrice, status, min_stock, max_stock, description) VALUES(:itemNumber, :itemName, :discount, :stock, :unitPrice, :status,:min_stock,:max_stock, :description)';
				$insertItemStatement = $conn->prepare($insertItemSql);
				$insertItemStatement->execute(['itemNumber' => $itemNumber, 'itemName' => $itemName, 'discount' => $discount, 'stock' => $quantity, 'unitPrice' => $unitPrice, 'status' => $status,'min_stock' => $min_stock,'max_stock' => $max_stock, 'description' => $description]);
